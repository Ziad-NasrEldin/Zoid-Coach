...
struct WeeklyPatternCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(alignment: .firstTextBaseline) {
                    Text("LEARNING STATUS")
                        ...
                        .foregroundStyle(Sumi.muted)
                    Spacer()
                    Text(presentation.learningStatusLabel)
                        .font(Sumi.label(9))
                        .sumiLabelTracking()
                        .foregroundStyle(Sumi.seal)
                        .accessibilityIdentifier("reviews.weekly.pattern.\(pattern.id).learning-status")
                }
                Text(presentation.learningBoundaryDetail)
                    .font(Sumi.body(11))
                    .foregroundStyle(Sumi.muted)
                    .fixedSize(horizontal: false, vertical: true)
                Text(presentation.learningSourceLabel)
                    .font(.system(size: 10, design: .monospaced))
                    .foregroundStyle(Sumi.muted)
            ...
            .accessibilityElement(children: .contain)
            .accessibilityIdentifier("reviews.weekly.pattern.\(pattern.id).learning-boundary")
