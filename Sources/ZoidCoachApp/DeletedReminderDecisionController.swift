...
struct DeletedReminderDecisionPresentation: Equatable, Sendable {
    ...
    init(decision: DeletedReminderDecision) {
        ...
        detail = "Apple Reminders no longer contains this task. \(list) \(due) The title shown here was retained. Notes, links, and other Reminder content were not copied."
        status = decision.state == .kept ? "KEPT IN LOCAL HISTORY" : "CHOOSE WHAT TO KEEP"
        let stableID = decision.sourceID.utf8.map { String(format: "%02x", $0) }.joined()
...
final class DeletedReminderDecisionController: ObservableObject {
    ...
    private func perform(success: String, operation: () throws -> Bool) {
        ...
        do {
            ...
            decisions = []
            errorMessage = "The choice was saved, but deleted Reminder choices could not be reloaded. Reload to see the saved state."
        }
    }
}
