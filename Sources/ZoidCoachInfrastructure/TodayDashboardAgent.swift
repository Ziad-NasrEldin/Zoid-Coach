...
public final class TodayDashboardAgent: @unchecked Sendable {
    ...
    public func snapshot(now: Date = Date()) throws -> TodaySnapshot {
        ...
        let sourceSnapshots = [
            SourceFreshnessSnapshot(sourceID: "reminders", state: "available", detail: "\(reminderSnapshots.count) normalized incomplete reminders", lastUpdatedAt: try reminders.lastUpdatedAt()),
        ...
        if command == .complete,
           ...
           let row = previousSnapshot?.taskRows.first(where: { $0.taskID == taskID }) {
            ...
            if let startedAt = activeBefore?.taskID == taskID ? activeBefore?.startedAt : nil, startedAt < now {
                ...
                _ = try learning.recordWorkWindowSample(workSample, timeZoneIdentifier: TimeZone.current.identifier)
                _ = try learning.updatePreferredWorkWindowAggregate(timeZoneIdentifier: TimeZone.current.identifier)
            }
        }
