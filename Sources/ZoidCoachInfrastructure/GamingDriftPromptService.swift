...
public final class GamingDriftPromptService: @unchecked Sendable {
    ...
    public func produce(
        ...
        guard try baselineIsComplete() else { return .suppressed(.observingBaseline) }
    ...
    private func baselineIsComplete() throws -> Bool {
        guard try tableExists("baseline_observation_days") else { return false }
        return try scalar(
            "SELECT COUNT(*) FROM baseline_observation_days WHERE coverage = 'complete';"
        ) >= 7
    }
