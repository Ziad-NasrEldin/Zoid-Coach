...
@Test
func migration44AddsPromptResolutionMetadataWithoutReclassifyingLegacyDismissals() throws {
    let databaseURL = temporaryDatabaseURL("v44-prompt-resolution-origin")
    defer { removeDatabaseFiles(at: databaseURL) }
    try execute(databaseURL, """
    CREATE TABLE schema_migrations (version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL);
    INSERT INTO schema_migrations(version, applied_at) VALUES (43, '2026-07-14T00:00:00Z');
    ...
    #expect(result.appliedVersions == [44])
    #expect(try columnExists(databaseURL, table: "prompt_episodes", column: "resolution_origin"))
    #expect(try columnExists(databaseURL, table: "prompt_episodes", column: "resolution_reason"))
