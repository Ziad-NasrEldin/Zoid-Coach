...
func deletedReminderPresentationUsesPrivacySafeCopyAndStableAccessibilityIDs() {
    ...
    #expect(presentation.detail.contains("Notes and Reminder content are not copied"))
    #expect(!presentation.detail.contains("Private client details"))
    #expect(presentation.status == "CHOOSE WHAT TO KEEP")
    #expect(presentation.rowAccessibilityID == "settings.reminders.deleted.reminder-1")
...
private func deletedDecision(state: DeletedReminderDecisionState = .pending) -> DeletedReminderDecision {
...
private final class DeletedReminderDecisionServiceStub: DeletedReminderDecisionServicing {
    ...
    func deletedReminderDecisions() throws -> [DeletedReminderDecision] { decisions }
