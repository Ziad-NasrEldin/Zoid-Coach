...
struct WorkCategoryTests {
    ...
    @Test("recognized category tools enter the work evidence pipeline without guessing ambiguous apps")
    func recognizedToolsBecomeWorkEvidence() {
        let classifier = BehaviorClassifier()

        #expect(classifier.classify(application: "Xcode") == .work)
        #expect(classifier.classify(application: "Figma") == .work)
        #expect(classifier.classify(application: "Zotero") == .work)
        #expect(classifier.classify(application: "Zoom") == .work)
        #expect(classifier.classify(application: "Finder") == .work)
        #expect(classifier.classify(application: "Safari") == .unknown)
    }
