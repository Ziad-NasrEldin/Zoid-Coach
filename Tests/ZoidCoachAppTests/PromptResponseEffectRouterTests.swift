...
@Test
func ambiguityKeepUnknownRoutesOnceWithoutCreatingACorrection() throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("zoid-ambiguity-router-\(UUID().uuidString)", isDirectory: true)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let databaseURL = root.appendingPathComponent("zoid.sqlite")
    let now = Date(timeIntervalSince1970: 1_800_000_000)
    let promptStore = try PromptInboxStore(databaseURL: databaseURL, now: { now })
    let ambiguity = try AmbiguousActivityPromptService(
    ...
    let episode = PromptEpisode(
        ...
        actions: [PromptAction(kind: .keepActivityUnknown, title: "Keep it unknown")],
        createdAt: now
    )
    let response = PromptResponse(
        ...
        actionToken: PromptResponseToken.make(promptID: episode.id, action: .keepActivityUnknown),
        surface: .dashboard,
        respondedAt: now
    )
    let router = PromptResponseEffectRouter(
        ...
        meetingArchive: try ScreenwatchArchive(databaseURL: databaseURL),
        promptStore: promptStore,
        ambiguousActivityPrompts: ambiguity
    )

    #expect(try router.apply(PromptResponseResult(
        response: response,
        episode: episode,
        wasApplied: true
    )) == .ambiguousActivityKeptUnknown(promptID: episode.id))
    #expect(try router.apply(PromptResponseResult(
        response: response,
        episode: episode,
        wasApplied: false
