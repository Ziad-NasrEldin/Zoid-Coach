...
VALUES ($OUTDATED_SCHEMA_VERSION, '2026-07-15T00:00:00Z');
...
CREATE TABLE schema_migrations(version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL);
INSERT INTO schema_migrations(version, applied_at)
VALUES ($EXPECTED_SCHEMA_VERSION, '2026-07-15T00:00:00Z');
...
SQL
    ...
    set_read_only_state "$database" >/dev/null
    assert_read_only "$database" >/dev/null
...
case "$COMMAND" in
    ...
    set-read-only) [[ -d "$BACKUP_ROOT" ]] || fail "capture original bytes first"; set_read_only_state "$DATABASE" ;;
