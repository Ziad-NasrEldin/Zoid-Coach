...
verify_reviewed_lineage() {
    ...
    contains_required_lines "$patch_ids" "$(printf '%s\n' "${REVIEWED_PATCH_IDS[@]}")" \
        || fail "signed commit is missing or alters a required reviewed patch"
...
if [[ "$APP" == "--self-test" ]]; then
    ...
    contains_required_lines "$reviewed_patches" "$reviewed_patches" || fail "reviewed patches rejected"
    ! contains_required_lines "$(sed '$d' <<<"$reviewed_patches")" "$reviewed_patches" \
        || fail "missing reviewed patch accepted"
    ! contains_required_lines "${reviewed_patches/50dde6b14d5387076062e13679c0060d711d3657/0000000000000000000000000000000000000000}" "$reviewed_patches" \
