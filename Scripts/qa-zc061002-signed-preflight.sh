...
readonly PARENT_TECHNICAL_CONTEXT="bd2431d223c031f2b523b4f11f3aedab5cb56999"
...
helper_pid=""
for _ in {1..40}; do
    service="$(launchctl print "gui/$(id -u)/$AGENT_LABEL" 2>/dev/null || true)"
    helper_pid="$(awk '/pid =/{print $3; exit}' <<<"$service")"
    [[ "$helper_pid" == <-> ]] && break
    sleep 0.2
done
readonly SERVICE="$service"
readonly HELPER_PID="$helper_pid"
...
database_is_open=0
for _ in {1..40}; do
    if lsof -a -p "$HELPER_PID" "$DATABASE" >/dev/null 2>&1; then
        database_is_open=1
        break
    fi
    sleep 0.2
done
(( database_is_open == 1 )) || fail "running helper does not hold the exact isolated database"
