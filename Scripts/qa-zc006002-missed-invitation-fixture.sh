...
usage() {
    print -u2 -- "usage: $0 <configure|assert-database|assert-notification|self-test> [path]"
    exit 2
}
...
self_test() (
    ...
    if "$JQ" -e --arg sentinel "$PRIVATE_SENTINEL" '
        [.notifications[] | select(
            ((.desired.title + " " + .desired.body) | test($sentinel; "i"))
        )] | length == 0
    ' "$root/private.json" >/dev/null; then
        fail "private notification content was accepted"
    fi
    print -- "PASS: ZC-006-002 fixture self-test rejects privacy leaks and duplicate-free contract drift"
)
