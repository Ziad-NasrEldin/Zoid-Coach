...
private func element(
    ...
    elements.first { candidate in
        (expectedRole == nil || role(candidate) == expectedRole)
            && labels(candidate).contains(exactLabel)
    }
}

private func findWithBoundedScroll(
    ...
    for page in 0...maximumScrollPages {
        ...
        if let match = element(in: current.elements, role: expectedRole, exactLabel: exactLabel) {
            return match
        }
...
private func assertInvalidResult(
    ...
    for _ in 0..<maximumPolls {
        ...
        let hasError = current.strings.contains(expectedError)
        ...
        if hasError, value == estimateCase.input, bool(field, kAXFocusedAttribute as CFString) == true {
            guard current.strings.contains("SAVE"), current.strings.contains("CANCEL") else {
                throw ProbeError.failure("invalid estimate did not keep correction controls available")
            }
            guard !current.strings.contains("Time estimate confirmed: 25 MIN") else {
...
private func assertValidResult(
    ...
    for _ in 0..<maximumPolls {
        ...
        if current.strings.contains(expected), editor == nil {
            let knownErrors = [
            ...
            guard knownErrors.allSatisfy({ !current.strings.contains($0) }) else {
...
do {
    ...
    switch phase {
    case "open":
        guard let trigger = try findWithBoundedScroll(
            in: window,
    ...
    case "submit":
        ...
        try assertPrivacy(window)
    case "persisted":
        guard let confirmed = try findWithBoundedScroll(
            in: window,
