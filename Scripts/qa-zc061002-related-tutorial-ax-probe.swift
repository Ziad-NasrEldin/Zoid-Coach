...
do {
    ...
    let safari = sheetElements.first {
        identifier($0) == "today.behavior-evidence.work-uncertainty.536166617269"
    }.map(exposedStrings) ?? []
    let work = sheetElements.first {
        identifier($0) == "today.behavior-evidence.category.work"
    }.map(exposedStrings) ?? []
    let research = sheetElements.first {
        identifier($0) == "today.behavior-evidence.work-category.research"
    }.map(exposedStrings) ?? []
