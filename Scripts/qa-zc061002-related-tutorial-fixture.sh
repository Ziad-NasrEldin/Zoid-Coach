...
self_test() {
    ...
    trap "rm -f -- ${temporary_database:q} ${temporary_database:q}-wal ${temporary_database:q}-shm" EXIT
...
SQL
    ...
    print -- "PASS: ZC-061-002 fixture self-test rejects overlap, context, relevance, privacy, and SQL failures while preserving foreign rows"
}
...
case "$ACTION" in
    ...
    self-test) self_test ;;
    *) fail "usage: $0 {prepare|verify|cleanup|verify-clean|self-test} [--database PATH] [--local-day YYYY-MM-DD]" ;;
esac
