...
assert_scope() {
    ...
    [[ "$(git -C "$REPOSITORY" rev-parse "$candidate^")" == "$PARENT" ]] || fail "candidate is not a direct child of ZC-062-003"
...
assert_runbook() {
    ...
    ' "$REPOSITORY/docs/ZC-062-004-SIGNED-QA-RUNBOOK.md" || fail "every runbook shell block must fail fast"
}

if [[ "${1:-}" == "--self-test" ]]; then
    is_sha "$PARENT" || fail "invalid parent SHA"
    git -C "$REPOSITORY" cat-file -e "$PARENT^{commit}" || fail "stacked parent unavailable"
    git -C "$REPOSITORY" merge-base --is-ancestor "$PARENT" HEAD || fail "worktree is not stacked on ZC-062-003"
    rg -Fq 'case "--once"' "$REPOSITORY/Sources/ZoidCoachAgent/AgentMain.swift" || fail "helper lacks bounded --once"
    assert_runbook
    "$SCRIPT_DIR/qa-zc062004-manual-tracking-outage-fixture.sh" self-test >/dev/null
    swift "$SCRIPT_DIR/qa-zc062004-manual-tracking-outage-ax-probe.swift" --self-test >/dev/null
    print -- "PASS: ZC-062-004 signed preflight self-test"
    exit 0
fi
...
[[ -d "$APP" && -f "$DATABASE" && -d "$SCREENWATCH_ROOT" ]] || fail "runtime inputs are missing"
...
[[ "${QA_ROOT:A}" == /private/tmp/zoid-666-zc062004-* && ! -L "$QA_ROOT" ]] || fail "unsafe QA root"
...
print -- "PASS: signed candidate, direct parent, exact helper, database, and isolated Screenwatch source are bound"
