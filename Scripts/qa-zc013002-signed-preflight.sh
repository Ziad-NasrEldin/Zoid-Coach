...
verify_lineage() {
    local expected="$1" scope head_scope entry expected_blob path actual_blob
    ...
    while IFS= read -r entry; do
        expected_blob="${entry%% *}"; path="${entry#* }"
        actual_blob="$(git -C "$REPOSITORY" rev-parse "$expected:$path")"
        [[ "$actual_blob" == "$expected_blob" ]] || fail "product blob changed during tooling: $path"
    done <<< "$PRODUCT_BLOBS"
}
