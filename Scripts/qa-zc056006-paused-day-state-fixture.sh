...
readonly TASK_ID="qa-zc056006-paused-day-task"
readonly TASK_TITLE="Resume the paused client brief"
...
seed_paused() {
    local foreign_active timestamp started_at ended_at
...
verify_paused() {
    assert_scalar \
        ...
        "1" \
        "owned local source task"
    assert_scalar \
        "SELECT COUNT(*) FROM daily_plan_entries WHERE day_key = '$LOCAL_DAY' AND reminder_id = '$TASK_ID' AND is_main_objective = 1;" \
        "1" \
        "owned main objective"
    assert_scalar \
        "SELECT COUNT(*) FROM task_execution_states WHERE task_id = '$TASK_ID' AND state = 'paused';" \
        "1" \
        "paused execution state"
    assert_scalar \
        "SELECT COUNT(*) FROM task_activity_intervals WHERE task_id = '$TASK_ID' AND ended_at IS NOT NULL;" \
        "1" \
        "closed tracked interval"
    assert_scalar \
        "SELECT COUNT(*) FROM task_activity_intervals WHERE ended_at IS NULL;" \
        "0" \
        "no active interval"
    printf 'PASS: ZC-056-006 paused day-state fixture is ready\n'
}
...
verify_clean() {
    ...
    printf 'PASS: ZC-056-006 owned rows were removed\n'
}

self_test() {
    local original_database="$DATABASE"
    local temporary_database
    temporary_database="$(mktemp -t zoid-zc056006-paused).sqlite"
...
INSERT INTO source_tasks(source_id, title, priority, is_completed, updated_at, source_kind)
VALUES ('foreign-task', 'Preserve me', 1, 0, '2026-07-15T00:00:00Z', 'local');
SQL
    require_schema
    seed_paused >/dev/null
    verify_paused >/dev/null
    cleanup_owned_rows
    ...
    printf 'PASS: ZC-056-006 fixture self-test preserves foreign rows\n'
}
...
case "$ACTION" in
    seed-paused) seed_paused ;;
    verify-paused) verify_paused ;;
    cleanup) cleanup_owned_rows; verify_clean ;;
    verify-clean) verify_clean ;;
    self-test) self_test ;;
    *) fail "usage: $0 {seed-paused|verify-paused|cleanup|verify-clean|self-test} [--database PATH] [--local-day YYYY-MM-DD]" ;;
esac
