...
DATABASE="$HOME/Library/Application Support/Zoid Coach/zoid-coach.sqlite"
SCREENWATCH_LOG="$HOME/screenwatch/days/$(date +%F)/log.jsonl"
INSTALLED_APP="${ZOID_COACH_INSTALL_ROOT:-$HOME/Applications}/Zoid Coach.app"
...
zoid_service="$(launchctl print "$USER_DOMAIN/com.ziadnasreldin.ZoidCoach.agent" 2>/dev/null)" \
    || fail "Zoid Coach agent is not registered"
...
if [[ -d "$INSTALLED_APP" ]]; then
    ...
    [[ "$agent_executable" == "$INSTALLED_APP/Contents/MacOS/ZoidCoachAgent" ]] \
        || fail "Zoid Coach agent is not running from the permanent installed app"
fi

[[ -f "$SCREENWATCH_LOG" ]] || fail "today's Screenwatch log is missing"
[[ -f "$DATABASE" ]] || fail "Zoid Coach database is missing"
...
(( latest_screenwatch_epoch - latest_source_epoch <= 240 )) \
    || fail "Zoid Coach ingestion is behind the latest Screenwatch record"

heartbeat="$(sqlite3 "$DATABASE" "SELECT last_success_at_utc FROM processing_checkpoints WHERE source_id = 'agent-runtime';")"
[[ -n "$heartbeat" ]] || fail "Zoid Coach agent heartbeat is missing"
heartbeat_epoch="$(date -j -u -f '%Y-%m-%dT%H:%M:%SZ' "$heartbeat" +%s 2>/dev/null)" \
    || fail "Zoid Coach agent heartbeat is invalid"
(( now_epoch - heartbeat_epoch <= 240 )) || fail "Zoid Coach agent heartbeat is stale"

echo "PASS: Screenwatch and Zoid Coach are loaded, running, and fresh"
