...
BASE="76149705b3a301fafa832102a2e599358a16ff25"
...
verify_reviewed_lineage() {
    local head scope reviewed_scope commit_count parent_line expected_parent_line
    local patch_ids expected_patch_ids
    head="$(git rev-parse HEAD)"
    git merge-base --is-ancestor "$BASE" "$head"
    git merge-base --is-ancestor "$REVIEWED_LINEAGE_TIP" "$head"
    expected_parent_line="$BASE $REVIEWED_LINEAGE_TIP"
