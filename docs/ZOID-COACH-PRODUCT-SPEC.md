...
7. Present a graded Dashboard prompt with explicit choices.
...
- Make one active task visible in the dashboard and summarize it in notifications.
...
- macOS notification authorization is available for intervention delivery.
...
- Verify notification authorization and present a test notification.
...
Installed Apps -----> App Inventory ----------+          v
                 ...
                 Recommendation Engine         Intervention Policy    Review Engine
                         ...
                         +------------------------------+------------------+
                                  ...
                                  |                     |                  |
                                  v                     v                  v
                              Dashboard          macOS Notifications         Local Reports
```
...
The Today dashboard is the persistent interactive surface.
...
5. Notification authorization and delivery check.
...
| macOS notification authorization | Optional but strongly recommended | Show timely prompts and compact actions | Dashboard and native notifications |
...
4. Present or update the dashboard active-task view.
...
- Notification authorization and test action.
...
| DashboardAdapter | Today view and action transport |
...
| Notification authorization unavailable | No timely notification | Keep decisions in the dashboard |
| Notification authorization revoked | Notification delivery fails | Keep prompts in the dashboard and show the permission repair path |
| Notification delivery is throttled | Timely prompts may be delayed | Keep unresolved prompts in the dashboard |
...
| Duplicate dashboard action arrives | Duplicate state mutation risk | Enforce command and token idempotency |
...
Dashboard action elements should expose labels, roles, state, and keyboard activation.
...
| Dashboard state refresh | Under 500 milliseconds after state change |
...
- Notification authorization and last delivery result.
...
- Notification authorization, delivery, action handling, and dashboard fallback.
...
#### Scenario F: notifications unavailable

Notification delivery fails before prompt presentation.
...
- Dashboard and notification ownership confirmed.
...
- Test notification presents after authorization.
...
- Dashboard presentation remains available when notifications fail.
...
| Dashboard action path is incomplete | Prompts cannot close the loop | Shared prompt actions and XPC validation |
...
| Dashboard or notification API changes | Integration breakage | Adapter boundaries and fallback surfaces |
