...
| 13 | Complete Today prompt inbox lifecycle | active | Prompt action feedback lane `codex/prompt-lifecycle-actions` | `ZC-038-005` and `ZC-038-006`; owned files are prompt pending/error presentation, one pure state model, focused UI-state and idempotency tests, and candidate evidence |
