...
| 6 | Finish Screenwatch healthy, invalid, alternate-folder, denial, and repair UX | verify | Screenwatch recovery candidate at `1fde15e`; schema-mismatch candidate pending verification | `ZC-049-008` now distinguishes complete changed-schema records from empty or unreadable streams, offers Repair, supports mixed-schema diagnosis, and exposes only aggregate counts, with focused proof in `.audit/runs/screenwatch-schema-mismatch/candidate/REPORT.md`; signed source-health verification remains |
