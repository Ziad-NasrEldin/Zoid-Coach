...
| 14 | Complete daily-plan task lifecycle | active | Local reschedule confirmation lane `codex/task-reschedule-date` | `ZC-020-008` and `ZC-034-010`; custom-estimate proof remains at `edc4e5e`; owned files are reschedule date state, Today confirmation UI, AppModel local-plan mutation, focused tests, and candidate evidence |
