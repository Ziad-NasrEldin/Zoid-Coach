...
| 13 | Complete Today prompt inbox lifecycle | verify | Prompt action feedback candidate at `8307a77` | `ZC-038-005` and `ZC-038-006`; selected-row progress, global duplicate prevention, durable cross-surface copy, focused state and prompt-store idempotency proof are recorded in `.audit/runs/prompt-action-feedback/candidate/REPORT.md` |
