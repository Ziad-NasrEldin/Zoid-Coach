...
| 0 | Complete the signed `Mark blocked` coaching journey | ready | Unowned after verifier `e7030f5` | `ZC-034-011`; the signed constrained Today window rendered the new task-change group, but its accessibility collection exposed no button children and `Mark blocked` remained unreachable without coordinate guessing. Replace the empty adaptive accessibility collection with directly reachable task-change buttons, then repeat the reason-sheet, invalid, Cancel, success, replacement-main, answered-history, relaunch, and unavailable-agent journey from `.audit/runs/prompt-blocker-reachability/verifier/REPORT.md` |
| --- | --- | --- | --- | --- |
