...
| 0 | Restore visible and actionable coaching controls in the Today prompt card | verify | Numeric-confirmation compatibility candidate | The signed empty collection was reproduced as strict envelope decoding silently dropping every action when the fixture stored confirmation flags as SQLite `0/1`; candidate preserves all six actions, `Mark blocked`, destructive confirmation, the existing adaptive grid, and sheet validation with focused proof in `.audit/runs/prompt-action-rendering/candidate/REPORT.md`; repeat the signed success, relaunch, and unavailable-agent journey |
| --- | --- | --- | --- | --- |
...
| 6 | Finish Screenwatch healthy, invalid, alternate-folder, denial, and repair UX | verify | Screenwatch recovery candidate at `1fde15e`; ingestion-control verifier at `6294c64` | `ZC-039-009` has the signed toggle and preservation copy plus focused policy, conflict, and source-gate proof; the isolated baseline did not import before the bounded runtime deadline, so one fresh signed run must establish a healthy baseline before proving pause across two loops, relaunch persistence, exactly-once resume without helper restart, source preservation, and an unrelated concurrent Settings save as recorded in `.audit/runs/screenwatch-ingestion-control/verifier/REPORT.md` |
...
| 14 | Complete daily-plan task lifecycle | verify | Prompt reschedule accepted at `75abc42`; prompt action compatibility candidate ready | `ZC-034-011` has the blocker-reason sheet, validation, ordered mutation, failure handling, replacement logic, and a focused fix for the signed fixture envelope that previously decoded to zero actions; repeat the capped success, failure, relaunch, and unavailable-agent journey recorded in `.audit/runs/prompt-action-rendering/candidate/REPORT.md` |
