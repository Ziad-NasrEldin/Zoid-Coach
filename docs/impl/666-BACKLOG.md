...
| 6 | Finish Screenwatch healthy, invalid, alternate-folder, denial, and repair UX | verify | Screenwatch recovery candidate at `1fde15e`; ingestion-control verifier at `6294c64`; schema-mismatch candidate pending verification | `ZC-039-009` has the signed toggle and preservation copy plus focused policy, conflict, and source-gate proof, but still needs a healthy signed baseline for the full pause and resume journey as recorded in `.audit/runs/screenwatch-ingestion-control/verifier/REPORT.md`. `ZC-049-008` now distinguishes complete changed-schema records from empty or unreadable streams, offers Repair, supports mixed-schema diagnosis, and exposes only aggregate counts, with focused proof in `.audit/runs/screenwatch-schema-mismatch/candidate/REPORT.md`; signed source-health verification remains. |
