...
Bind every acceptance phase to one exact clean signed integration commit containing the complete repaired candidate `e130399c95ade28598e1020b31e8eaaf7fcb34de`.

```sh
REPO="$PWD"
APP="/absolute/path/to/Zoid 666 QA E2E.app"
QA_ROOT="/private/tmp/zoid-666-zc029010-runtime"
...
git merge-base --is-ancestor e130399c95ade28598e1020b31e8eaaf7fcb34de "$EXPECTED_SIGNED_COMMIT"
ZOID_COACH_PACKAGE_MODE=qa "$REPO/Scripts/verify-package.sh" \
  "$APP" --expected-commit "$EXPECTED_SIGNED_COMMIT" --require-clean
APP_PLIST="$APP/Contents/Info.plist"
AGENT_PLIST="$(find "$APP/Contents/Library/LaunchAgents" -name '*.plist' -type f -print -quit)"
test "$(plutil -extract ZoidCoachQARunRoot raw -o - "$APP_PLIST")" = "$QA_ROOT"
test "$(/usr/libexec/PlistBuddy -c 'Print :LSEnvironment:ZOID_COACH_QA_RUN_ROOT' "$APP_PLIST")" = "$QA_ROOT"
test "$(/usr/libexec/PlistBuddy -c 'Print :EnvironmentVariables:ZOID_COACH_QA_RUN_ROOT' "$AGENT_PLIST")" = "$QA_ROOT"
```

Do not continue if the installed package identity, repaired-candidate ancestry, clean build identity, or embedded app and helper QA roots differ.
After the helper starts, require its executable to hold this exact `DATABASE` open before changing Settings or collecting Today evidence.
