...
- dashboard authorization and one-way test activity presentation over the local WebSocket endpoint.
...
- Native dashboard action callbacks.
...
Visible WhatsApp text --/                                       |               |
                                                           ...
                                                           SQLite event log   action outbox
                                                                  |               |
                                                                  v               v
Zoid Coach app <------ authenticated XPC ------> ZoidCoachAgent -> EventKit / dashboard / Notifications
```
...
| `ZoidCoachInfrastructure` | Library | SQLite, EventKit, Screenwatch, Vision OCR, AI providers, notifications, dashboard, XPC, and system lifecycle |
...
Add one shared prompt inbox whose state feeds the dashboard, dashboard, and notifications.
...
Extend the dashboard integration with native action descriptors and callbacks as already decided in the product specification.
...
Onboarding must separately explain and verify Reminders, Calendar, Notifications, Screen Recording, Accessibility, Automation, dashboard, and launch-at-login permissions.
...
Use a fake dashboard endpoint and fake notification center.
...
- dashboard action-contract spike with a confirmed native or temporary adapter path.
